...
void ALightTrigger_03::OnEntering(UPrimitiveComponent * OverlappedComponent, AActor * OtherActor, UPrimitiveComponent * OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult)
{
	...
	*/
	...
	UProperty* Property = OtherActor->GetClass()->FindPropertyByName(TEXT("Special"));
	if (Property) {
		bool* ptrValue = Property->ContainerPtrToValuePtr<bool>(OtherActor); // get property as bool 
...
void ALightTrigger_03::LightOperator(){
	for (AActor* Actor : Lights) {
		ASpotLight* s = Cast<ASpotLight>(Actor); // cast spotlight as an actors from Lights array
		
		if (s != nullptr) s->SpotLightComponent->SetVisibility(LightsVisibility);
		UE_LOG(LogTemp, Warning, TEXT(" -            light:  %s"), *(Actor->GetName()));
	}
